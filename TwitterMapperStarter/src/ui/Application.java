            ...
            public void mouseMoved(MouseEvent e) {
                ...
                for (MapMarker marker : markers) {
                    if (marker instanceof MapMarkerColor) {
                        MapMarkerColor m = (MapMarkerColor) marker;
                        Status status = m.getStatus();
                        map().setToolTipText(status.getText());
                    }
                    if (marker instanceof MapMarkerImage) {
                        MapMarkerImage m = (MapMarkerImage) marker;
                        Status status = m.getStatus();
                        map().setToolTipText("<html>" + "<img src=" + (status.getUser().getProfileImageURL()) + ">" + status.getText() + "</html>");
                    }
                }
            }
        });
